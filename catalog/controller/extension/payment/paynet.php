...
class ControllerExtensionPaymentPaynet extends Controller
{
    ...
    public function paymentform()
    {
        ...
        require_once(DIR_APPLICATION . 'controller/payment/paynetconfig.php');
		...
		if(isset($this->request->post['session_id']) AND $this->request->post['token_id']){	
			...
			try{
				...
				$chargeParams->token_id = $this->request->post['token_id'];
								$chargeParams->amount = strval($total_cart*100);
	...
	public function getPaymentButton(){
		...
		<script type="text/javascript"
			...
			data-amount= '.(int)($total_cart*100).'
			data-image="'.$logo_url.'"
			...
			data-no_instalment="'.($this->config->get('paynet_installments') == 'on' ? 'true': 'false').'"
